...
var sort = function sort(events, _ref) {
  ...
  return events.sort(function(a, b) {
    ...
    if (startA === startB) {
      return (
        +(0, _accessors.accessor)(b, endAccessor) -
        +(0, _accessors.accessor)(a, endAccessor)
      )
    }
...
var getSlot = function getSlot(event, accessor, min, totalMin) {
  return (
    event &&
    positionFromDate((0, _accessors.accessor)(event, accessor), min, totalMin)
  )
}
...
function getStyledEvents(_ref7) {
  ...
  var _loop = function _loop() {
    ...
    var _getChildGroups = getChildGroups(
        idx,
    ...
    idx +=
      1 +
      siblings.length +
      childGroups.reduce(function(total, group) {
